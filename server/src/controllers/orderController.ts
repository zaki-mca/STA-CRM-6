...
class OrderController extends BaseController {
  ...
  update = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (items && Array.isArray(items)) {
        ...
        const existingItems = existingItemsResult.rows as OrderItemRow[];
        ...
        if (removeUnlistedItems) {
          const itemIds = (items as OrderItem[]).filter((item) => item.id).map((item) => item.id);
  ...
  createInvoice = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const providerResult = await query('SELECT id FROM providers WHERE id = $1', [provider_id]);
        ...
        SELECT i.*, 
          ...
          (
            SELECT COALESCE(json_agg(json_build_object(
              ...
              'category_name', COALESCE((SELECT pc.name FROM product_categories pc WHERE pc.id = prod.category_id), 'Uncategorized'),
              'brand_id', COALESCE(prod.brand_id, ''),
              'brand_name', COALESCE((SELECT pb.name FROM product_brands pb WHERE pb.id = prod.brand_id), 'Unbranded'),
      ...
      return next(new AppError(`Failed to create invoice: ${error instanceof Error ? error.message : 'Unknown error'}`, 500));
    }
  });
