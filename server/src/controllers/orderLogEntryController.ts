...
class OrderLogEntryController extends BaseController {
  constructor() {
  ...
  addOrderToLog = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const logCheck = await query('SELECT id FROM order_logs WHERE id = $1', [order_log_id]);
    ...
    try {
      const result = await query(
        `INSERT INTO order_log_entries (order_log_id, order_id, notes)
         ...
         RETURNING *`,
        [order_log_id, order_id, notes || null]
      );
      ...
      return next(new AppError('Failed to add order to log. The order may already be in this log.', 400));
    }
  });
  ...
  addOrdersBatch = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!entries || !Array.isArray(entries) || entries.length === 0) {
      return next(new AppError('At least one order entry is required', 400));
    }
    ...
    try {
      ...
      return next(new AppError('Failed to add orders to log. Check for duplicate orders or invalid IDs.', 400));
    }
  });
  ...
  getEntriesByLogId = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Check if order log exists
    const logCheck = await query('SELECT id FROM order_logs WHERE id = $1', [logId]);
  ...
  removeOrderFromLog = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const entryCheck = await query('SELECT id FROM order_log_entries WHERE id = $1', [id]);
    
    if (entryCheck.rows.length === 0) {
      return next(new AppError('Order log entry not found with this ID', 404));
    }
    
    // Get the log ID before deleting the entry
    const logResult = await query('SELECT order_log_id FROM order_log_entries WHERE id = $1', [id]);
    const order_log_id = logResult.rows[0].order_log_id;
    
    // Delete the entry
    await query('DELETE FROM order_log_entries WHERE id = $1', [id]);
...
export default new OrderLogEntryController(); 