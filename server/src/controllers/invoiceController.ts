...
class InvoiceController extends BaseController {
  ...
  getAll = catchAsync(async (req: Request, res: Response) => {
    try {
      ...
      invoicesResult.rows.forEach((row: any) => {
  ...
  getById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!invoiceResult.rows.length) {
        return next(new AppError(`No invoice found with ID: ${id}`, 404));
      }
      
      // Create the invoice object
      const invoiceRow = invoiceResult.rows[0] as DBInvoiceRow;
      ...
      try {
        ...
        // Process items
        for (const item of itemsResult.rows as DBInvoiceItemRow[]) {
  ...
  create = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!client_id && !provider_id) {
        return next(new AppError('Either client_id or provider_id must be provided', 400));
      }
      
      if (client_id && provider_id) {
        return next(new AppError('Cannot specify both client_id and provider_id', 400));
      }
      ...
      try {
        ...
        if (isUuid) {
          ...
          entityResult = await query(`SELECT id FROM ${entityTable} WHERE id = $1`, [entityId]);
        } else if (idColumnType === 'uuid') {
          // Try UUID cast for UUID columns
          try {
            entityResult = await query(`SELECT id FROM ${entityTable} WHERE id = $1::uuid`, [entityIdParam]);
          } catch (e) {
          ...
          if (!isNaN(Number(entityIdParam))) {
            entityResult = await query(
              `SELECT id FROM ${entityTable} WHERE id = $1`, 
          ...
          entityResult = await query(`SELECT id FROM ${entityTable} WHERE id = $1`, [entityIdParam]);
        }
      ...
      if (entityResult.rows.length === 0) {
        ...
        return next(new AppError(`${provider_id ? 'Provider' : 'Client'} not found with ID: ${entityIdParam}`, 404));
      }
      ...
      // Insert invoice
      const invoiceResult = await query(fullQuery, params);
      ...
      for (const item of items) {
        ...
        const productResult = await query('SELECT id, price FROM products WHERE id = $1', [item.product_id]);
      ...
      const formattedInvoice = {
        ...
        subtotal: (invoiceData.items || []).reduce((sum: number, item: any) => sum + (parseFloat(item.total) || 0), 0),
      };
      ...
      if (dbError.code === '23503') { // Foreign key violation
        return next(new AppError(`Referenced entity not found: ${dbError.detail}`, 400));
      } else if (dbError.code === '23505') { // Unique violation
        return next(new AppError(`Duplicate value: ${dbError.detail}`, 400));
      } else if (dbError.code === '23514') { // Check violation
        return next(new AppError(`Constraint violation: ${dbError.constraint} - ${dbError.detail}`, 400));
      }
      
      return next(new AppError(`Database error: ${dbError.message}`, 500));
    }
  });
  ...
  update = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const invoiceCheck = await query('SELECT * FROM invoices WHERE id = $1', [id]);
      
      if (invoiceCheck.rows.length === 0) {
        await query('ROLLBACK');
        return next(new AppError('Invoice not found with this ID', 404));
      }
      ...
      if (items && Array.isArray(items)) {
        ...
        const currentItems = await query('SELECT * FROM invoice_items WHERE invoice_id = $1', [id]);
        
        // Map of current items by product_id for easy lookup
        const currentItemsByProductId = new Map();
        currentItems.rows.forEach((item: InvoiceItemRow) => {
        ...
        if (req.body.removeUnlistedItems === true) {
          for (const item of currentItems.rows) {
            if (!processedItemIds.has(item.id)) {
              ...
              await query('DELETE FROM invoice_items WHERE id = $1', [item.id]);
            }
          }
        }
      }
      ...
      if (dbError.code === '23503') { // Foreign key violation
        ...
        return next(new AppError(`Invalid input format: ${dbError.message}`, 400));
      }
  ...
  delete = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      // Check if invoice exists
      const invoiceCheck = await query('SELECT id FROM invoices WHERE id = $1', [id]);
  ...
  getByStatus = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  getSimplified = catchAsync(async (req: Request, res: Response) => {
    try {
      ...
      const invoices = result.rows.map((row: DBInvoiceRow) => ({
        id: row.id,
