...
async function fixInvoiceProviders() {
  ...
  try {
    ...
    const providersResult = await query('SELECT id, name FROM providers ORDER BY name LIMIT 1');
    ...
    if (providersResult.rows.length === 0) {
      ...
      const updateAllResult = await query(`
        UPDATE invoices
        SET provider_id = $1
      ...
      if (invalidResult.rows.length > 0) {
        ...
        const updateResult = await query(`
          UPDATE invoices
          SET provider_id = $1
          WHERE id IN (${invalidResult.rows.map((row: any) => `'${row.id}'`).join(',')})
          RETURNING id, invoice_number
        `, [defaultProvider.id]);
    ...
    countResult.rows.forEach((row: any, index: number) => {
