...
class BrandController extends baseController_1.BaseController {
    constructor() {
        ...
        this.bulkCreate = (0, errorHandler_1.catchAsync)(async (req, res) => {
            ...
            try {
                // Parse the file
                const brands = await (0, fileParser_1.parseFile)(req.file);
                ...
                try {
                    ...
                    for (const brand of brands) {
                        ...
                        const checkResult = await client.query(`SELECT * FROM ${this.tableName} WHERE name = $1`, [brand.name]);
                        if (checkResult.rows.length > 0) {
                            duplicates.push(brand.name);
                            continue;
                        }
                        // Insert the new brand
                        const result = await client.query(`INSERT INTO ${this.tableName} (name, description) VALUES ($1, $2) RETURNING *`, [brand.name, brand.description || '']);
