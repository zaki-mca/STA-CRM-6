        ...
        this.bulkUpload = (0, errorHandler_1.catchAsync)(async (req, res) => {
            ...
            try {
                ...
                if (validRecords.length === 0) {
                    throw new errorHandler_1.AppError('No valid category records found in file', 400);
                }
                // Insert records into database
                const insertedItems = [];
                for (const record of validRecords) {
                    // Check if category already exists
                    const existingCategory = await (0, db_1.query)(`SELECT * FROM ${this.tableName} WHERE name = $1`, [record.name]);
