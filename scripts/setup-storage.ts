...
async function setupStorage() {
  ...
  for (const bucket of buckets) {
    ...
    // Check if error indicates the bucket doesn't exist (404 Not Found)
    // The error message for non-existent bucket typically contains "does not exist" or similar
...
async function setupPublicBucketPolicies(bucketName: string) {
...
async function setupPrivateBucketPolicies(bucketName: string) {
